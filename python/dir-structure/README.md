...
1.  **Context Creation:** turning your existing project structure into a text file to show an LLM.
...
*   **Gitignore Integration (NEW):** Respects your project's `.gitignore` file during scanning, automatically excluding build artifacts, logs, secrets, and temp files from the generated tree.
...
4.  The script will:
    ...
    *   **Move** existing files to their new locations if they are currently sitting in the root.
...
### Safety & Exclusion Mechanisms
*   **Sanitization:** Leading slashes (`/var/www`) are stripped to force relative paths (`var/www`). This prevents the script from trying to write to `C:\` or `/` on Linux.
...
*   **Dynamic Exclusions (Gitignore):** The script parses your `.gitignore` using wildcard matching (`fnmatch`). If your gitignore contains `*.log`, the generator will skip every file ending in `.log` automatically.
...
This script is provided "as-is". It is open source and free to modify. 
